...
export function seedEvents(dayKey) {
  const make = (title, start, end, color, repeat = null) => ({
  ...
  return [
    make('Sunday School', 11 * 60 + 40, 12 * 60 + 30, PINK, 'weekly'),
    make('Sacrament Meeting', 13 * 60, 14 * 60, PINK, 'weekly'),
    make('Almuerzo', 13 * 60 + 15, 14 * 60, PURPLE, 'weekly'),
    make('T-Charts', 13 * 60, 14 * 60, PURPLE, 'weekly'),
    make('Estudio de Idioma', 14 * 60, 15 * 60, PURPLE, 'weekly'),
    make('Companionship Study', 15 * 60, 16 * 60, PURPLE, 'weekly'),
